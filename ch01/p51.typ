...
#solution[
  (d) $arrow.double$ (a):
  $
    nabla times (- nabla V) = nabla times (pdv(V, x)vu(x) + pdv(V, y)vu(y) + pdv(V,z)vu(z)) = \ 
    = curlc(pdv(V, x), pdv(V,y), pdv(V,z)) = \
    = (pdv(V, y,z, deg:2) - pdv(V, z, y, deg:2))vu(x) + (pdv(V, z,x, deg:2) - pdv(V, x, z, deg:2))vu(y) + (pdv(V, x,y, deg:2) - pdv(V, y, x, deg:2))vu(z) = 0 " "square
  $
